...
const publicDir = fs_1.default.existsSync(path_1.default.join(__dirname, '../public'))
    ...
    : (fs_1.default.existsSync(path_1.default.join(process.cwd(), 'public'))
...
app.get(['/api/services', '/api/tests'], async (req, res) => {
    try {
        const [rows] = await db_js_1.pool.query('SELECT * FROM services WHERE is_active = 1 ORDER BY id ASC');
        res.json(rows);
    }
    catch (err) {
...
app.get('/api/packages', async (req, res) => {
    try {
        const [rows] = await db_js_1.pool.query('SELECT * FROM packages WHERE is_active = 1 ORDER BY id ASC');
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/articles', async (req, res) => {
    try {
        const [rows] = await db_js_1.pool.query('SELECT * FROM articles WHERE is_published = 1 ORDER BY publish_date DESC');
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.get('/api/testimonials', async (req, res) => {
    try {
        const [rows] = await db_js_1.pool.query('SELECT * FROM testimonials WHERE is_featured = 1 ORDER BY id ASC');
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.post(['/api/bookings', '/api/appointments'], async (req, res) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
    `;
        const [result] = await db_js_1.pool.execute(sql, [
            booking_code,
            patient_name,
            email || '',
            phone,
...
app.post('/api/payments/verify', async (req, res) => {
    try {
        ...
        if (booking_code) {
            await db_js_1.pool.execute('UPDATE bookings SET payment_id = ?, payment_status = ? WHERE booking_code = ?', [paymentId, 'paid', booking_code]);
        }
        else if (booking_id) {
            await db_js_1.pool.execute('UPDATE bookings SET payment_id = ?, payment_status = ? WHERE id = ?', [paymentId, 'paid', booking_id]);
        }
        res.json({
            success: true,
...
app.post('/api/inquiries', async (req, res) => {
    try {
        ...
        const [result] = await db_js_1.pool.execute('INSERT INTO inquiries (name, email, phone, subject, message) VALUES (?, ?, ?, ?, ?)', [name, email, phone || '', subject || 'General Inquiry', message]);
...
const handleLogin = async (req, res) => {
    try {
        ...
        const [rows] = await db_js_1.pool.query('SELECT * FROM admins WHERE username = ? OR email = ?', [username, username]);
        if (rows.length === 0) {
            // Default fallback credentials for easy evaluation
            if ((username === 'admin' || username === 'admin@healthlab.in') && password === 'admin123') {
                const token = jsonwebtoken_1.default.sign({ id: 1, username: 'admin', role: 'superadmin' }, JWT_SECRET, { expiresIn: '7d' });
        ...
        const token = jsonwebtoken_1.default.sign({ id: admin.id, username: admin.username, role: admin.role }, JWT_SECRET, { expiresIn: '7d' });
        res.json({
            success: true,
            token,
            admin: {
...
app.get('/api/admin/stats', async (req, res) => {
    try {
        const [totalBookings] = await db_js_1.pool.query('SELECT count(*) as count FROM bookings');
        const [pendingBookings] = await db_js_1.pool.query("SELECT count(*) as count FROM bookings WHERE status = 'pending'");
        const [confirmedBookings] = await db_js_1.pool.query("SELECT count(*) as count FROM bookings WHERE status = 'confirmed'");
        const [completedBookings] = await db_js_1.pool.query("SELECT count(*) as count FROM bookings WHERE status = 'completed'");
        const [inquiries] = await db_js_1.pool.query("SELECT count(*) as count FROM inquiries WHERE status = 'new'");
        res.json({
...
const handleGetBookings = async (req, res) => {
    try {
        ...
        const [rows] = await db_js_1.pool.query(query, params);
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
const handleTrackBooking = async (req, res) => {
    try {
        const query = (req.query.query || req.query.code || req.query.phone || '').trim();
        if (!query) {
    ...
    `;
        const [rows] = await db_js_1.pool.query(sql, [
            query.toUpperCase(),
            query,
            cleanDigits,
            `%${cleanDigits || query}%`,
...
const handlePatientRegister = async (req, res) => {
    try {
        ...
        const [existing] = await db_js_1.pool.query('SELECT * FROM patients WHERE phone = ? OR (email != "" AND email = ?)', [phone, email || '']);
        if (existing.length > 0) {
            return res.status(400).json({ error: 'An account with this mobile phone or email already exists. Please sign in.' });
        }
        const [result] = await db_js_1.pool.query('INSERT INTO patients (patient_name, phone, email, password_hash) VALUES (?, ?, ?, ?)', [resolvedName, phone, email || '', password]);
        const newPatient = { id: result.insertId, patient_name: resolvedName, phone, email: email || '' };
        const token = jsonwebtoken_1.default.sign(newPatient, JWT_SECRET, { expiresIn: '30d' });
        res.json({
            success: true,
...
const handlePatientLogin = async (req, res) => {
    try {
        ...
        const [rows] = await db_js_1.pool.query(`SELECT * FROM patients 
       WHERE (phone = ? OR email = ? OR (LENGTH(?) >= 7 AND REPLACE(REPLACE(REPLACE(phone, ' ', ''), '-', ''), '+', '') = ?)) 
         AND password_hash = ? 
       LIMIT 1`, [identifier, identifier, cleanDigits, cleanDigits, password]);
        if (rows.length === 0) {
            return res.status(401).json({ error: 'Invalid phone/email or password' });
        }
        const patient = rows[0];
        const token = jsonwebtoken_1.default.sign({ id: patient.id, patient_name: patient.patient_name, phone: patient.phone, email: patient.email }, JWT_SECRET, { expiresIn: '30d' });
        res.json({
            success: true,
...
const handlePatientAppointments = async (req, res) => {
    try {
...
const handleUpdateNotes = async (req, res) => {
    try {
        ...
        if (doctor_prescription !== undefined) {
            updates.push('doctor_prescription = ?');
            params.push(doctor_prescription);
        }
        ...
        params.push(id);
        await db_js_1.pool.execute(`UPDATE bookings SET ${updates.join(', ')} WHERE id = ?`, params);
...
const handleUpdateStatus = async (req, res) => {
    try {
        ...
        await db_js_1.pool.execute('UPDATE bookings SET status = ? WHERE id = ?', [status || 'confirmed', id]);
...
app.delete('/api/admin/bookings/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await db_js_1.pool.execute('DELETE FROM bookings WHERE id = ?', [id]);
...
app.get('/api/admin/inquiries', async (req, res) => {
    try {
        const [rows] = await db_js_1.pool.query('SELECT * FROM inquiries ORDER BY created_at DESC');
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.put('/api/admin/inquiries/:id/status', async (req, res) => {
    try {
        ...
        await db_js_1.pool.execute('UPDATE inquiries SET status = ? WHERE id = ?', [status, id]);
...
app.post('/api/tools/bmi', (req, res) => {
    ...
    let category = 'Normal';
    let risk = 'Low';
    let recommendedTests = ['Routine Complete Blood Count (CBC)', 'Lipid Profile Screen'];
    if (bmi < 18.5) {
...
app.get('/api/guides/test-prep', (req, res) => {
    res.json({
        success: true,
        guides: [
            {
...
app.get('/api/admin/patients', async (req, res) => {
    ...
    `;
        ...
        const [rows] = await db_js_1.pool.execute(query, params);
        res.json(rows);
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
app.delete('/api/admin/patients/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await db_js_1.pool.execute('DELETE FROM patients WHERE id = ?', [id]);
...
app.get('/api/site-content', async (req, res) => {
    try {
        const [rows] = await db_js_1.pool.query('SELECT section_key, content_json FROM site_content');
        const result = {};
...
app.get('/api/site-content/:section', async (req, res) => {
    try {
        ...
        const [rows] = await db_js_1.pool.query('SELECT content_json FROM site_content WHERE section_key = ?', [section]);
        if (rows.length === 0) {
...
app.put('/api/admin/site-content/:section', async (req, res) => {
    try {
        ...
        await db_js_1.pool.query(`INSERT INTO site_content (section_key, content_json) VALUES (?, ?)
       ON DUPLICATE KEY UPDATE content_json = ?`, [section, jsonStr, jsonStr]);
...
async function initSiteContentTable() {
    ...
    `);
        ...
        for (const [key, val] of Object.entries(defaults)) {
            ...
            await db_js_1.pool.query(`INSERT INTO site_content (section_key, content_json) VALUES (?, ?)
         ON DUPLICATE KEY UPDATE section_key = section_key`, [key, jsonStr]);
        }
